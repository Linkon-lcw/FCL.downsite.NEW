...
import { handleError } from './selectorDefaults.js';
...
export default class loadContent {
        ...
        return new Promise((resolve, reject) => {
          function read() {
            reader.read().then(({ done, value }) => {
              if (done) {
                // 将所有chunk合并为完整文本
                const html = new TextDecoder().decode(utils.concatChunks(chunks));
                resolve(html);
                return;
              }
   ...
   */
  static xf_loadHtmlContent(htmlContent, container) {
   ...
   * @param {Element} container - 用于加载内容的 DOM 元素容器
   */
  static async xf_loadHtmlContentFromUrl(url, container) {
    ...
    try {
      ...
      const htmlContent = await this.xf_getHtmlContent(url, (loaded, total) => {
        // 更新进度信息
      ...
      if (htmlContent) {
        this.xf_loadHtmlContent(htmlContent, container); // 加载 HTML 内容到指定容器
      }
