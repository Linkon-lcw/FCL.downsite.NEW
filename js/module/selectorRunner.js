...
import {
  defaultCreateSelectElement,
  defaultCreateDescriptionElement,
  defaultCreateDownloadElement,
  defaultRenderError,
} from './selectorDefaults.js';

const PREFIX = '选择器引擎：';
const COLUMN_HEADERS = ['操作', '架构', '描述', '大小', '显示名称', 'URL'];

export default class SelectorRunner {
  constructor(options) {
    const {
      containerId,
      dataSource,
    ...
    console.log(`${PREFIX}初始化，容器ID：${containerId}`);
    ...
    if (!this.container) {
      console.error(`${PREFIX}找不到容器：${containerId}`);
      return;
    }

    this.callbacks = {
      onCreateSelectElement: onCreateSelectElement || defaultCreateSelectElement,
      onCreateDescriptionElement: onCreateDescriptionElement || defaultCreateDescriptionElement,
      onCreateDownloadElement: onCreateDownloadElement || defaultCreateDownloadElement,
      onItemSelect,
      onDownload,
      onRenderError: onRenderError || ((msg, lv, ctr) => defaultRenderError(
        msg, lv, ctr,
        () => this.enableAllControls(),
        (lv) => this.clearLevelElements(lv),
      )),
      onLevelChange,
    };

    this.sysInfo = sysInfo.detectSystemInfo();
    console.log(`${PREFIX}系统信息检测完成`, this.sysInfo);
  }

  async start() {
    if (!this.container) return;
    console.log(`${PREFIX}开始加载根级数据`);
  ...
  async loadLevel(source, level, inheritedFilter) {
    ...
    try {
      const rawItems = await loadContent.fetchItems(source);
      console.log(`${PREFIX}层级 ${level}：获取到 ${rawItems.length} 条数据`);

      const items = await transformApiData.transformDataIfNecessary(
        rawItems,
      ...
      if (this.pendingDataTransform !== undefined) {
        ...
        this.pendingDataTransform = undefined;
      }

      if (!Array.isArray(items)) {
        console.warn(`${PREFIX}层级 ${level}：数据格式异常，期望数组`);
        this.callbacks.onRenderError('数据格式异常：期望数组', level, this.container);
        return;
      }

      const isBottom = this.isBottomLevel(items);
      ...
      this.callbacks.onRenderError(error.message, level, this.container);
    }
  }
  ...
  addSelectEventListeners(select, items, level, descDiv, inheritedFilter) {
    select.addEventListener('change', async () => {
      ...
      if (isNaN(selectedIndex) || selectedIndex < 0 || selectedIndex >= items.length) {
        console.warn(`${PREFIX}层级 ${level} 选中无效索引：${select.value}`);
        return;
      }
      ...
      if (selectedItem.description) {
        ...
        this.tryEnableControls(selectedItem);
      }

      if (selectedItem.desUrl) {
        console.log(`${PREFIX}${selectedItem.name}：从 ${selectedItem.desUrl} 加载描述`);
        try {
          const content = await loadContent.fetchItems(selectedItem.desUrl, 'text');
          descDiv.innerHTML = content;
          if (selectedItem.desJsUrl) {
            console.log(`${PREFIX}${selectedItem.name}：加载描述用脚本 ${selectedItem.desJsUrl}`);
          ...
          console.error(`${PREFIX}${selectedItem.name}：加载描述失败：`, error);
        }
      }

      if (this.callbacks.onItemSelect) {
        const result = this.callbacks.onItemSelect(selectedItem, level);
        if (result) {
          console.log(`${PREFIX}${selectedItem.name}：外部 onItemSelect 回调已接管，跳过默认逻辑`);
          return;
        }
      }
  ...
  tryEnableControls(selectedItem) {
    if (this.isBottomLevel([selectedItem])) {
      console.log(`${PREFIX}${selectedItem.name}：描述加载完毕且为最底层，重新启用控件`);
      this.enableAllControls();
    }
  }
  ...
  async handleItemSelection(selectedItem, nextLevel, descDiv, inheritedFilter) {
    ...
    this.clearLevelElements(nextLevel);
  }
  ...
  autoSelectOption(select, items, level) {
    ...
    console.log(`${PREFIX}层级 ${level}：默认选项索引：${defaultIndex}，实际选择索引：${autoSelectIndex}`);
  ...
  renderDownloadButtons(items, level, filter) {
    ...
    validItems.forEach(item => {
      ...
      const row = this.callbacks.onCreateDownloadElement(
        item,
    ...
    table.classList.value = 'mdui-table download-buttons-container';
    ...
    columnIndicesToKeep.forEach(colIndex => {
      ...
      th.textContent = COLUMN_HEADERS[colIndex];
    ...
    if (hasMatchedArch) {
      ...
      matchDesc.textContent = '已匹配到当前架构，请留意表格中绿色行（仅供参考，如安装失败请选择通用（all）架构）。';
    ...
    this.enableAllControls();
    console.log(`${PREFIX}层级 ${level}：下载按钮渲染完成`);
  }

  disableAllControls() {
    console.log(`${PREFIX}禁用全部控件`);
    ...
    if (this.forceStopLoadBtnId) {
      const forceStopBtn = document.getElementById(this.forceStopLoadBtnId);
      if (forceStopBtn) {
